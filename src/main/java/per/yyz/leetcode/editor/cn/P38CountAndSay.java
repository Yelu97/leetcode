    ...
    class Solution {
        ...
        public String convert(String result) {
            ...
            for (int i = 1; i < result.length() + 1; i++) {
                if (i == result.length()) {
                    resultStr.append(i - start).append(result.charAt(start));
                    break;
                }
                if (result.charAt(i) != result.charAt(start)) {
                    resultStr.append(i - start).append(result.charAt(start));
                    start = i;
                }
            }
        ...
        public String convert01(String result) {
            StringBuilder resultStr = new StringBuilder();
            Queue<Character> queue = new LinkedList<>();
            queue.add(result.charAt(0));
            for (int i = 1; i < result.length(); i++) {
                if (!queue.contains(result.charAt(i))) {
                    resultStr.append(queue.size() + queue.peek().toString());
                    queue.clear();
                }
